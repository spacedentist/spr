...
| `stackingMode`       |                                   | How stacked pull requests are set up: `base-branches`, `chain` or `github-stack`[^stacking] | `base-branches` with `squash`, `chain` with `merge` |
...
[^stacking]: See [stacking modes](../user/stack.md#stacking-modes). The combination of `mergeMethod` `merge` and `stackingMode` `base-branches` is not allowed.
